...
class AccountCash(models.Model):
    ...
    def action_confirm(self):
        ...
        for cash_line in self.lines:
            if not cash_line.account_id:
                raise UserError('Bạn chưa cấu hình Tài khoản chi phí hoặc Tài khoản doanh thu trong Xuất hóa đơn của sản phẩm %s' %(str(cash_line.product_id.name)))
        return True
    ...
    def action_carrying(self):
        ...
        for line in self.lines:
            if not line.account_id:
                raise UserError('Bạn chưa cấu hình Tài khoản chi phí hoặc Tài khoản doanh thu trong Xuất hóa đơn của sản phẩm %s' %(str(line.product_id.name)))
        ...
        if self.session_id:
            ...
            for line in self.lines:
                ...
                line.sudo().partner_id.x_loyal_total = line.partner_id.x_loyal_total + amount
