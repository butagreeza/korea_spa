...
class InvoiceMakePayment(models.TransientModel):
    ...
    check_invisible_date_due = fields.Boolean(string='Check', compute='get_check_invisible_date_due', default=False)

    @api.depends('amount')
    def get_check_invisible_date_due(self):
        for detail in self:
            if detail.amount < detail.invoice_id.residual:
        ...
        if order_id and len(order_id) == 1:
            ...
            if order_id.session_id.config_id.journal_debt_id.id not in order_id.session_id.config_id.journal_loyal_ids.ids\
                ...
                self.invoice_id.partner_id.update({
                    'x_loyal_total': self.invoice_id.partner_id.x_loyal_total + self.amount - (self.amount/100*self.journal_id.card_swipe_fees),
                ...
                if self.journal_id.id in self.session_id.config_id.journal_loyal_ids.ids:
                    self._allocation_revenua(self.amount - (self.amount/100*self.journal_id.card_swipe_fees), order_id, self.session_id)
