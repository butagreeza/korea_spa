...
class PosOrder(models.Model):
    ...
    def action_customer_signature(self):
        if self.state not in ['customer_comment', 'to_confirm'] :
            raise except_orm("Cảnh báo!", 'Trạng thái của đơn hàng đã thay đổi. Vui lòng tải lại trang hoặc F5')
    ...
    def process_customer_signature(self):
        ...
        if self.state == 'paid' or self.state == 'invoiced':
            raise except_orm("Cảnh báo!", ("Trạng thái của đơn hàng đã thay đổi. Vui lòng F5 hoặc tải lại trang"))
        # if self.x_debt:
        #     self.state = 'invoiced'
        # else:
        #     self.state = 'paid'
    ...
    # Thêm job tự động nhắc lịch đòi nợ KH
    ...
    def create_actvity_due(self, inv_ids):
        ...
        #     raise except_orm('Cảnh báo!', ('Bạn chưa cấu hình ngày nhắc lịch ngày thanh toán công nợ.'))
