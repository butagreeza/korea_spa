...
class PosMakePayment(models.TransientModel):
    ...
    def compute_amount_by_vc(self, order):
        amount = self.x_amount if self.x_amount <= order.amount_total else order.amount_total
        return amount
    ...
    check_invisible_date_due = fields.Boolean(string='Check', default=False, compute='get_check_invisible_date_due')


    @api.depends('journal_id')
    def get_check_invisible_date_due(self):
        for detail in self:
            if detail.journal_id.code.upper() == 'GN':
                detail.check_invisible_date_due = True
            else:
                detail.check_invisible_date_due = False

    @api.onchange('journal_id')
    def _onchange_date_due(self):
        if self.journal_id:
            if self.journal_id.code.upper() == 'GN':
                self.date_due = date.today() + timedelta(7)
    ...
    def onchange_vc_code(self):
        if self.x_vc_code:
            ...
            vc._invalidate_vc_code(order.partner_id.id)
            # Nếu là Voucher
            if vc.x_discount == 0:
    ...
    def check(self):
        ...
        #Huyenbv Thêm nếu thanh toán là ghi nợ thì thêm ngày đòi nợ vào đơn hàng
        order.date_due = self.date_due
        ...
        # Thanh toán bằng phiếu mua hàng
            ...
            if self.x_vc_code:
                ...
                # Nếu là Voucher
                if vc.x_discount == 0:
                    self.amount = self.compute_amount_by_vc(order)
                # Ngược lại là Coupon
                else:
                    to_discount = order.amount_total * vc.x_discount / 100.0
                    if vc.x_amount and to_discount > vc.x_amount:
        ...
        if not order.x_pos_partner_refund_id and not name_therapy:
            ...
            for line in order.lines:
                ...
                if line.x_is_gift == True or line.discount == 100:
                    ...
                    key = '%s_amount' % line.product_id.id
                    # korea cho phép bán sp trên 2 dòng
