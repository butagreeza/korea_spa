...
class PosSession(models.Model):
    ...
    x_amount_total = fields.Float(string='Amount', compute='_onchange_amount_total', store=True, readonly=True, copy=False)

    @api.depends('statement_ids', 'x_amount_total')
    def _onchange_amount_total(self):
        for detail in self:
            for line in detail.statement_ids:
                detail.x_amount_total += line.total_entry_encoding
