...
class ResPartner(models.Model):
    ...
    pos_order_count = fields.Integer(string='Đơn hàng', compute='get_count_pos_order')
    date_due = fields.Date(string='Date due', compute='_compute_date_due_partner')

    @api.depends('name')
    def _compute_date_due_partner(self):
        for detail in self:
            query = """
                    SELECT * FROM account_invoice
                    WHERE partner_id = %s and state = 'open'
                    ORDER BY date_due
                    """
            self._cr.execute(query, ([detail.id]))
            res = self._cr.dictfetchall()
            for line in res:
                detail.date_due = line['date_due']
                break

    @api.depends('pos_order_count')
    def get_count_pos_order(self):
        for line in self:
            pos_order_ids = self.env['pos.order'].search([('partner_id', '=', line.id)])
            if pos_order_ids:
                line.pos_order_count = len(pos_order_ids)
