...
class PosChangePayment(models.Model):
    ...
    def action_change_payment_done(self):
        ...
        self.amount = self.amount_total - registed
        if self.amount != 0:
            raise UserError('Chưa thanh toán hết đơn hàng')
        self.x_status = 'done'
        self.state = 'invoiced'
        ...
        #sửa lại phân bổ thanh toán
        payment_allocation_id = self.env['pos.payment.allocation'].search([('order_id', '=', self.id)], limit=1)
        payment_allocation_id.write({
            'state': 'draft',
            'default_unlink': False,
        })
        payment_allocation_id.unlink()
        if self.x_type != '2':
