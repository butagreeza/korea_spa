...
class PosUserMaterial(models.Model):
    ...
    def action_confirm(self):
        ...
        if count == 0:
            ...
            self.state = 'exported'
            picking_material_ids = self.env['pos.user.material'].search([('using_service_id', '=', self.using_service_id.id)])
            if len(picking_material_ids) == len(picking_material_ids.filtered(lambda picking:picking.state in ['exported', 'done'])):
    ...
    def action_supervisor_confirm(self):
        ...
        if self.adjust_active == False:
            self.state = 'exported'
            picking_material_ids = self.env['pos.user.material'].search(
                [('using_service_id', '=', self.using_service_id.id)])
            if len(picking_material_ids) == len(
                    picking_material_ids.filtered(lambda picking: picking.state in ['exported', 'done'])):
                self.using_service_id.state = 'working'
            # SangsLA thêm ngày 3/10/2018 Thêm using_service vào form khi chung của khách hàng
            pos_sum_digital_obj = self.env['pos.sum.digital.sign'].search(
                [('partner_id', '=', self.customer_id.id), ('state', '=', 'draft'), ('session_id', '=', self.using_service_id.pos_session_id.id)], limit=1)
            if pos_sum_digital_obj:
                self.using_service_id.update({'x_digital_sign_id': pos_sum_digital_obj.id})
            else:
                pos_sum_digital_obj = self.env['pos.sum.digital.sign'].create({
