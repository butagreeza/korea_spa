...
class ReportPartnerFeedback(models.TransientModel):
    _name = 'report.partner.feedback'

    from_date = fields.Date("From Date")
    to_date = fields.Date("To Date")

    @api.multi
    def create_report_partner_feedback(self):
        param_obj = self.env['ir.config_parameter']
        url = param_obj.get_param('birt_url')
        if not url:
            raise ValidationError(_(u"Bạn phải cấu hình birt_url"))
        report_name = "report_partner_feedback.rptdesign"
    ...
    @api.multi
    def create_report_partner_feedback_excel(self):
        param_obj = self.env['ir.config_parameter']
        url = param_obj.get_param('birt_url')
        if not url:
            raise ValidationError(_(u"Bạn phải cấu hình birt_url"))
