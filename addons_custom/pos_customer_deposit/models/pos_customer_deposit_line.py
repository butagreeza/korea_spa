...
class PosCustomerDepositLine(models.Model):
    ...
    is_sign_order = fields.Boolean(string='Sign order', default=False, compute='compute_sign_order')
    ...
    @api.multi
    def compute_sign_order(self):
        for line in self:
            user_id = self.env['res.users'].search([('id', '=', self.env.uid)], limit=1)
            config_id = user_id.x_pos_config_id
            if config_id.x_is_sign_order:
                line.is_sign_order = True
    ...
    def action_rate(self):
        if self.state != 'confirm':
            raise except_orm('Cảnh báo!', ("Trạng thái bản ghi đã bị thay đổi vui lòng F5 hoặc tải lại trang"))
    ...
    def action_done(self):
        ...
        if self.type == 'deposit':
            ...
            # self.partner_id.x_loyal_total = self.partner_id.x_loyal_total + self.amount - (self.amount/100*self.journal_id.card_swipe_fees)
            ...
            user_id = self.env['res.users'].search([('id', '=', self.env.uid)], limit=1)
            config_id = user_id.x_pos_config_id
            if config_id.x_allocation_deposit:
                if self.deposit_id.journal_id.id in config_id.journal_loyal_ids.ids:
                    raise UserError(
                        'PHẢI bỏ hình thức cấn trừ đặt cọc trong mục Phương thức ghi nhân doanh thu trước khi phân bổ doanh thu cho đơn đặt cọc')
                revenue_id = self._auto_allocation(self.amount - (self.amount/100*self.journal_id.card_swipe_fees))
        ...
        if self.type == 'cash':
            ...
            if config_id.x_allocation_deposit:
                ...
                if self.deposit_id.journal_id.id in config_id.journal_loyal_ids.ids:
                    raise UserError('PHẢI bỏ hình thức cấn trừ đặt cọc trong mục Phương thức ghi nhân doanh thu trước khi phân bổ doanh thu cho đơn đặt cọc')
                revenue_id = self._auto_allocation(-(self.amount - (self.amount/100*self.journal_id.card_swipe_fees)) + self.charge_refund)
            # revenue_id.amount_allocated = 0
            # revenue_id.amount_res = -(self.amount - self.charge_refund - (self.amount/100*self.journal_id.card_swipe_fees))
                self.revenue_id = revenue_id.id
            # hoantien
            move_lines = []
            debit_move_vals = {
    ...
    def _auto_allocation(self, amount):
        ...
        vals = {
            'order_id': False,
            'pos_customer_deposit_line_id': self.id,
        ...
        if amount > 0:
            note = 'KH đặt cọc'
            revenua_id.type = 'deposit'
        else:
            note = 'Hoàn tiền KH'
            revenua_id.type = 'cash'
        count_nvtv = 0
