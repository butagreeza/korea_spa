        ...
        if model == 'res.partner':
            ...
            if not group_erp_manager:
                if group_revenue_control:
                    domain = domain
                elif group_cost_accountant or group_customer_care or group_leader_customer_care \
                        or group_revenue_accountant or group_business_manager \
                        or group_receptionist or group_cashier \
                        or group_leader_consultant \
                        or group_leader_shop or group_inventory_accounting\
                        or group_doctor:
                    phone = ''
                    for item in domain:
                        if phone: break
                    ...
                    if phone:
                        ...
                        if partner:
                            ...
                            domain += ['|', ('x_crm_team_id', 'in', team_ids), ('x_telesales_id', '=', user.id)]
                    else:
                        domain += ['|', ('user_id', '=', user.id), ('x_telesales_id', '=', user.id)]

                elif group_leader_telesales:
                        domain += ['|', ('x_telesales_id', '=', user.id), ('x_stage_code', '=', 'won')]
                elif group_leader_social_network_administrator:
                        domain += [('x_stage_code', '!=', 'won')]
                elif group_telesales or group_leader_telesales:
                        domain += [('x_telesales_id', '=', user.id)]
                elif group_social_network_administrator:
                        domain += [('create_uid', '=', user.id), ('x_stage_code', '!=', 'won')]
                else:
                    domain = [('id', '=', 0)]
