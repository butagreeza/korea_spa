...
class RefungInvoiceMakePayment(models.Model):
    ...
    def onchange_invoice_id(self):
        ...
        ids = []
        p_ids = []
        # [('id', 'in', [x.id for x in self.invoice_ids])],
        invoice_ids = self.env['account.invoice'].sudo().search([('partner_id', '=', self.partner_id.id)])
        payment_ids = self.env['account.payment'].sudo().search(
            [('payment_date', '=', self.date), ('partner_id', '=', self.partner_id.id), ('invoice_ids', 'in', (x.id for x in self.invoice_id)),
             ('payment_type', '=', 'inbound'), ('partner_type', '=', 'customer'),
             ('state', 'not in', ('draft', 'cancelled')), ('x_debt_settlement', '=', True)])
        for line in invoice_ids:
            ids.append(line.id)
                    ...
                    if self.payment_id.journal_id.id in journal_loyal_ids:
                        ...
                        self.env.cr.execute("""
                                                ...
                                                where partner_id = %s and order_id = %s and amount_total = %s and date::date = %s
                                                                """, (
                            self.payment_id.partner_id.id, self.order_id.id, self.amount - (self.amount/100*self.journal_id.card_swipe_fees),
            ...
            for line in self.payment_id.invoice_ids:
                ...
                if payment_allocation:
                    ...
                    for line_allocation in payment_allocation.payment_allocation_ids:
                        line_allocation.unlink()
                    payment_allocation.write({
                        'state': 'draft',
                        'default_unlink': False,
                    })
                    payment_allocation.unlink()
        #     hết Sáng la . Đẩy code module pos_payment_allocation thì bỏ comment đi

            # Cập nhật lại ngày đòi nợ trên hóa đơn
            self.invoice_id.date_due = self.date_due
        else:
            raise except_orm("Cảnh báo", ("Không tìm thấy phương thức thanh toán hợp lệ.Vui lòng kiểm tra lại."))
