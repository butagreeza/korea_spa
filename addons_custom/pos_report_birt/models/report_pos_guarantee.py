...
class ReportPosGuarantee(models.TransientModel):
    _name = 'report.pos.guarantee'
    ...
    @api.multi
    def create_report_pos_guarantee_excel(self):
        param_obj = self.env['ir.config_parameter']
        url = param_obj.get_param('birt_url')
        if not url:
            raise ValidationError(_(u"Bạn phải cấu hình birt_url"))
