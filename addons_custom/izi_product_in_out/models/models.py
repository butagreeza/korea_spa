...
class ev_product_delivery(models.Model):
    ...
    def _default_partner_request(self):
        return self.uid
    ...
    partner_request_id = fields.Many2one('res.users', string='Requester', default=lambda self: self.env.user)
    ...
    def get_loss_location(self):
        ...
        if not loss_location:
            ...
            # raise except_orm("Thông báo CẤU HÌNH", "Không tìm thấy kho Inventory loss (kho mất mát) trong hệ thống.\n"
    ...
    def do_check_inventory(self):
        ...
        if not self.delivery_lines or len(self.delivery_lines) == 0:
            raise except_orm("Thông báo", "Xin hãy nhập ít nhất một sản phẩm để xử lý")
        state = 'available'
    ...
    def create_out_picking(self):
        ...
        if len(move_lines) > 0:
            picking_args = {
                ...
                'x_reason': self.delivery_reason.id,
    ...
    def unlink(self):
        for delivery in self:
            if delivery.state != 'draft':
                raise UserError('Phiếu xuất nhập chi phí khác trạng thái nháp nên không thể xóa!')
...
class InventoryTransferLine(models.Model):
    ...
    @api.onchange('product_id')
    def _onchange_delivery_reason(self):
        for line in self:
            line.delivery_reason = line.transfer_id.delivery_reason