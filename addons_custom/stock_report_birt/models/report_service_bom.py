...
class ReportServiceBom(models.TransientModel):
    _name = "report.service.bom"
    ...
    def create_report_service_bom(self):
        ...
        # param_str = param_str1 + param_str2 + product_id + param_str3 + partner_id + param_str4 + service_id
        return {
            'type': 'ir.actions.act_url',
            'url': url + "/report/frameset?__report=report_korea/" + report_name,
            'target': 'new',
        }

    @api.multi
    def create_report_service_bom_export(self):
        param_obj = self.env['ir.config_parameter']
        url = param_obj.get_param('birt_url')
        if not url:
            raise ValidationError(_(u"Bạn phải cấu hình birt_url"))
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': url + "/report/frameset?__report=report_korea/" + report_name + '&__format=xlsx',
            'target': 'self',
        }
