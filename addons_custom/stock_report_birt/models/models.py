...
class Reportimportandexportinventory(models.TransientModel):
    ...
    type = fields.Selection([('out', 'Out'), ('in', 'In')], string='Type')
    ...
    def create_report_import_and_export_inventory(self):
        ...
        if not self.type:
            ...
            param_str1 += self.type
        param_str = "&from_date=" + str(from_date) + "&to_date=" + str(to_date) + "&location_id=" + str(location_id.id) + param_str1
        return {
